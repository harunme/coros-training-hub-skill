...
description: "从 COROS 高驰训练中心获取用户的运动数据（跑步、健走、越野跑等）和训练日程计划，包括距离、配速、心率、训练负荷等指标，以及未来训练计划安排。当用户询问跑步数据、训练记录、高驰运动历史、训练计划、训练日程、训练趋势，或需要分析训练效果时触发。本 skill 只返回 COROS 原始数据，不对配速区间或心率区间做主观推算。"
---
...
2. 浏览器 F12 -> Application/Cookies，复制 `CPL-coros-token` 的值（约 32 字符，如 `LH6VND3MA20NQ63CHSKPMYR361T66IQY`）
...
当用户想了解近期训练规律、跑量趋势、周频次等时触发。不依赖训练日程 API，从近期运动记录实时分析。只返回 COROS 原始数据，不对配速区间或心率区间做主观推算。

调用方式：

```bash
python3 scripts/coros.py --token <accessToken> analyze [--days <N>]
```
